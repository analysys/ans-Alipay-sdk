...
var callbackArr = [];
var callbackfirstArr = [];
// sdk是否准备就绪
var isReady = function isReady() {
    return isGetServerTime && isInitConfig && isGetNetWork;
};
// 执行缓存函数
function implementAallbackArr() {
    if (isReady()) {
        ...
        if (callbackfirstArr && callbackArr.length) {
            callbackfirstArr.forEach(function (o) {
...
function autoPageView() {
    if (config.auto === true) {
        var self = getCurrentPage();
        var pageId = self.__wxExparserNodeId__;
        // 防止多次自动触发pageView事件
...
function hookMethods(methods) {
    ...
    // 监听页面离开
    var statusClear = function statusClear() {
        var self = getCurrentPage();
        var pageId = self.__wxExparserNodeId__;
        if (eventAttribute.pageview.state[pageId]) {
...
function AppFn(app) {
    ...
    appFnApply(app, 'onShow', function () {
        ...
        if (!eventAttribute.startup.state) {
            ready(startUp).apply(void 0, arg);
