...
var publicAttrs = ['$appid', '$appname', '$lib', '$lib_version', '$platform', '$is_login', '$brand', '$model', '$os', '$os_version', '$browser', '$browser_version', '$network', '$screen_width', '$screen_height', '$language', '$debug', '$time_zone', '$session_id', '$is_time_calibrated'];
// utm相关属性
var utmAttrs = ['$utm_campaign_id', '$utm_campaign', '$utm_medium', '$utm_source', '$utm_content', '$utm_term'];
// 预制事件列表与事件属性
var events = {
    $startup: __spreadArrays(utmAttrs, ['$is_first_day', '$is_first_time' //首次访问，只在startUp
    ]),
    $end: ['$duration' //使用时长
    ],
    $pageview: __spreadArrays(utmAttrs, ['$is_first_day', '$referrer', '$startup_time', '$url', '$url_domain', '$share_id', '$share_path', '$share_level']),
    page_close: ['$url', '$referrer', '$url_domain', 'pagestaytime'],
    $alias: ['$original_id'],
    $share: [],
    $getPresetProperties: ['$first_visit_time'],
    $user_click: ['$element_content', '$element_id', '$element_type', '$element_name', '$element_function', '$url', '$url_path', '$is_first_day'],
...
function lifecycleHook(stateKey, hookName, data) {
    if (!hookState[stateKey] && globalWindow.AnalysysAgent) {
        var hook = globalWindow.AnalysysAgent[hookName];
        if (isFunction(hook)) {
            hook(data);
        }
...
var lifecycleHook$1 = {
     ...
     */
    onReady: function onReady(config) {
        if (globalWindow.AnalysysAgent) {
            var onReady = globalWindow.AnalysysAgent.onReady;
    ...
    // 初始化完成后首次发送预制启动事件之前
    onBeforeStartUp: function onBeforeStartUp(data) {
        lifecycleHook('isBeforeStartUp', 'onBeforeStartUp', data);
    },
    // 初始化完成后首次发送预制启动事件发送成功之后
    onAfterStartUp: function onAfterStartUp(data) {
        ...
        if (!isGetNetWork || !isGetServerTime) {
            ...
            if (!isReady) {
                Promise.all([setNetwork(), getServerTime()]).then(function () {
                    ...
                    lifecycleHook$1.onReady(config);
 ...
 */
var userClickAttrs = {};
function setUserClickAttrs(attrs) {
...
function startUp() {
    ...
    if (args.length === 2 && args[1] === 'onShow') {
        lifecycleHook$1.onBeforeStartUp(res);
    }
    sendData(res, function () {
        // 初始化完成后首次发送预制启动事件成功后，触发生命周期钩子
        lifecycleHook$1.onAfterStartUp(res);
    });
    // 是否已发送首次用户属性，没有则发送
...
function userClick() {
    ...
    if (args.length > 0 && args[0] && isObject(args[0]) && (args[0].type == "tap" || args[0].type == "longtap" || args[0].type == "longpress")) {
        var element_function = args[1];
        // 获取uniapp写的小程序函数方法名
        try {
            var eventOpts = args[0].target.dataset.eventOpts;
            if (eventOpts) {
        ...
        // 设置全埋点属性
        var elementAttrMap = args[0] && args[0].currentTarget && args[0].currentTarget.dataset ? args[0].currentTarget.dataset : {};
...
function track(eventName, eventAttrs) {
...
function share(toShareProperties, properties) {
    ...
    var sendShare = function sendShare() {
        // 获取上报数据模块
        var res = fillData('$share');
        var shareObj = {
            '$share_id': xwho,
...
var PageHook = ['onLoad', 'onShow', 'onReady', 'onHide', 'onUnload', 'onPullDownRefresh', 'onReachBottom', 'onShareAppMessage', 'onShareTimeline', 'onAddToFavorites', 'onPageScroll', 'onResize', 'onTabItemTap', 'onSaveExitState'];

// 原有生命周周期函数的封装 ，不影响小程序原有生命周期函数执行
function appFnApply(obj, Fn, toFn) {
    var oldFn = obj[Fn];
    if (oldFn) {
...
function hookMethods(methods) {
    appFnApply(methods, 'onLoad', function (query) {
        var page = getCurrentPage();
        eventAttribute.pageview.query[page.$id] = query;
    });
    appFnApply(methods, 'onShow', function () {
        // 是否自动采集pageview事件
        if (config.auto === true) {
            var page = getCurrentPage();
            // 防止多次自动触发pageView事件
    ...
    // 监听页面离开
    var statusClear = function statusClear(isOnUnload) {
        if (isOnUnload === void 0) {
            isOnUnload = false;
        }
        var page = getCurrentPage();
        if (eventAttribute.pageview.state[page.$id]) {
    ...
    appFnApply(methods, 'onHide', function () {
        statusClear();
    });
    appFnApply(methods, 'onUnload', function () {
        statusClear(true);
    });
...
function AppFn(app) {
    appFnApply(app, 'onLaunch', function (option) {
        setPathParams(option);
    });
    // 自动上报启动事件
    appFnApply(app, 'onShow', function () {
    ...
    // 全局监听app onHide事件
    appFnApply(app, 'onHide', function () {
        if (eventAttribute.startup.state) {
            end();
...
function ComponentFn(component) {
